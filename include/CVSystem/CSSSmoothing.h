...
namespace CVSystem
{
	...
	public:
		...
		// old version
		void Init1();
		void SmoothCurve1(std::vector<CVSystem::MyPoint>& curve);

		// improved version
		void Init2();
		void SmoothCurve2(std::vector<CVSystem::MyPoint>& curve, cv::Mat distance_img);
	...
	private:
		...
		bool ShouldDoSmoothing(std::vector<CVSystem::MyPoint> oriCurve, int n, cv::Mat distance_img);
		...
		void getdXY(std::vector<CVSystem::MyPoint> x, 
						int n, 
						double sigma, 
						CVSystem::MyPoint& gx, 
						std::vector<double> g);
		...
		void getdXYcurve(std::vector<CVSystem::MyPoint> x, 
							double sigma, 
							std::vector<CVSystem::MyPoint>& gx, 
							std::vector<double> g,
							cv::Mat distance_img);
	};
}
